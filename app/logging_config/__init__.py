...
LOGGING_CONFIG = {
    ...
    'formatters': {
        'standard': {
            ...
            'format': '[%(asctime)s] [%(process)d] %(remote_addr)s [%(levelname)s]: %(message)s'
        },
    ...
    'handlers': {
        ...
        'file.handler.user_activity': {
            'class': 'logging.handlers.RotatingFileHandler',
            'formatter': 'standard',
            'filename': os.path.join(config.Config.LOG_DIR,'user_activity.log'),
            'maxBytes': 10000000,
            'backupCount': 5,
        }
    },
    'loggers': {
        ...
        'user_activity': {  # if __name__ == '__main__'
            'handlers': ['file.handler.user_activity'],
            'level': 'INFO',
            'propagate': False
        },
